...
MCP_ENDPOINT=http://180.101.21.13:8080/gateway/cmdb/mcp
EOF
...
case "${TARGET_ENV}" in
    development)
        echo "📋 开发环境配置:"
        echo "   - MCP 服务: cmdb-mcp-server（http://180.101.21.13:8080/gateway/cmdb/mcp）"
        ;;
    production)
        echo "📋 生产环境配置:"
        echo "   - MCP 服务: cmdb-mcp-server（http://180.101.21.13:8080/gateway/cmdb/mcp）"
...
echo "2. 在 StarAgent 平台注册 MCP 服务 cmdb-mcp-server，地址 http://180.101.21.13:8080/gateway/cmdb/mcp"
