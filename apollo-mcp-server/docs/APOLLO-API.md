...
| 服务 | 端口 | 用途 | 认证 |
|------|------|------|------|
| **ConfigService** | 8080 | 给**应用 SDK** 拉取配置用（`/configs/...`） | 无 |
| **OpenAPI**（Portal 开放平台接口） | 8070 | 给**第三方程序/管理工具**调用 | Token |

MCP 服务作为第三方工具，统一走 **OpenAPI（8070）** 即可，不需要也不应该直接访问 8080 的 ConfigService。两个接口都是 Apollo 默认提供的。
